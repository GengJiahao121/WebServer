...
int setnonblocking( int fd ) {
    int old_option = fcntl( fd, F_GETFL );  // 获取一个文件描述符（fd）的当前文件状态标志
    int new_option = old_option | O_NONBLOCK;   // 将文件描述符设置为非阻塞模式
...
void modfd(int epollfd, int fd, int ev) {
    epoll_event event;
    event.data.fd = fd;
    event.events = ev | EPOLLET | EPOLLONESHOT | EPOLLRDHUP;    // 当套接字上的对端关闭连接或关闭时，epoll会生成EPOLLRDHUP事件。
...
void http_conn::close_conn() {
    if(m_sockfd != -1) {
        removefd(m_epollfd, m_sockfd);  // 将该用户的sockfd从epoll事件检测数组中移除
        m_sockfd = -1;
                     ...
                     int close_log, string user, string passwd, string sqlname, int epoll_et)
{
    ...
    printf("http_conn::init doc_root = %s\n", doc_root);
...
void http_conn::init()
{
    ...
    /*
    ************************************************
    为什么使用bzero会清空doc_root和数据库相关地址中的内容呢？
    ************************************************
...
http_conn::LINE_STATUS http_conn::parse_line() {
    ...
    for ( ; m_checked_idx < m_read_idx; ++m_checked_idx ) { 
        ...
        if ( temp == '\r' ) {   // 只有'\r'没有'\n'的情况
            ...
            // 到末尾了还没有/n说明数据不完整，可能数据没有完全到达
            if ( ( m_checked_idx + 1 ) == m_read_idx ) {
                return LINE_OPEN;
            
            // 说明一个完整的行结束，把\r和\n替换成\0\0
            } else if ( m_read_buf[ m_checked_idx + 1 ] == '\n' ) {
                m_read_buf[ m_checked_idx++ ] = '\0';   // '\0是结束符'
...
// 将请求体的内容保存到http_conn对象的m_string数组中
...
http_conn::HTTP_CODE http_conn::process_read() {
    LINE_STATUS line_status = LINE_OK;  // 从状态机，刚开始时初始化为LINE_OK
    HTTP_CODE ret = NO_REQUEST; // 主状态机，刚开始时初始化为请求不完整，需要继续读取客户数据
    char* text = 0; // 
...
{
    // printf("http_conn::HTTP_CODE http_conn::do_request() : %d\n", strlen(sql_name));
    
    strcpy( m_real_file, doc_root );
    int len = strlen( doc_root );

    const char *p = strrchr(m_url, '/');    // 查找字符串 m_url 中最后一个斜杠字符 '/' 的位置，并将结果存储在指针 p 中
    ...
    if (cgi == 1 && (*(p + 1) == '2' || *(p + 1) == '3'))
    {
        ...
        if (*(p + 1) == '3'){
            ...
            if (users.find(name) == users.end()){
                ...
                users.insert(pair<string, string>(name, password));     // 这行代码使用C++的std::map容器的insert函数，将一对键-值对（key-value pair）插入到users中。
                m_lock.unlock();
                if (!res){
...
void http_conn::process() {
    ...
    // 生成响应
    bool write_ret = process_write( read_ret ); // 将数据写入写缓冲区
    if ( !write_ret ) {
        close_conn();
    }
